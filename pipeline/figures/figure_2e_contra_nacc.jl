...
ij = 15
conmod_norm_contra_nacc = hcat([
    (
        kernel_norm[mouse]["NAcc"][dms_ipsi_map[mouse]][:, 4] .- 
        kernel_norm[mouse]["NAcc"][dms_ipsi_map[mouse]][:, 1] 
    ) 
    for mouse in mouse_ids 
]...)

beh_contra_nacc = hcat([
    behavior_weight[mouse].avg[:, beh_dms_ipsi_map[mouse]] * beh_multiplier_ipsi[mouse] for mouse in mouse_ids
]...)

neural_vals = conmod_norm_contra_nacc[:, ij]
beh_vals = beh_contra_nacc[:, ij]
...
plt.savefig("contra_nacc_beh_neu_cor.pdf", bbox_inches="tight", transparent=true)