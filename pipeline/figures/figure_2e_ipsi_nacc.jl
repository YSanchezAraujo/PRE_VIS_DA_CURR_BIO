...
ij = 15
conmod_norm_ipsi_nacc = hcat([
    (
        kernel_norm[mouse]["NAcc"][dms_contra_map[mouse]][:, 4] .- 
        kernel_norm[mouse]["NAcc"][dms_contra_map[mouse]][:, 1] 
    ) 
    for mouse in mouse_ids 
]...)
...
neural_vals = conmod_norm_ipsi_nacc[:, ij]
beh_vals = beh_ipsi_nacc[:, ij]
...
plt.savefig("ipsi_nacc_beh_neu_cor.pdf", bbox_inches="tight", transparent=true)