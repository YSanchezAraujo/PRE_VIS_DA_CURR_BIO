...
ij = 15
conmod_norm_contra_dms = hcat([
    (
        kernel_norm[mouse]["DMS"][dms_contra_map[mouse]][:, 4] .- 
        kernel_norm[mouse]["DMS"][dms_contra_map[mouse]][:, 1] 
    ) 
    for mouse in mouse_ids 
]...)

beh_contra_dms = hcat([
    behavior_weight[mouse].avg[:, beh_dms_contra_map[mouse]] * beh_multiplier_contra[mouse] for mouse in mouse_ids
]...)

neural_vals = conmod_norm_contra_dms[:, ij]
beh_vals = beh_contra_dms[:, ij]
...
plt.savefig("contra_dms_beh_neu_cor.pdf", bbox_inches="tight", transparent=true)